...
public class ElanFrame2 extends JFrame implements ActionListener,
	 ...
	 * actions */
    private HashMap<String, MenuAction> menuActions = new HashMap<String, MenuAction>();
    ...
    private HashMap<Object, Action> registeredActions = new HashMap<Object, Action>();
	...
	private ElanMenuItem addCommentMI;
     ...
     */
    public ElanFrame2(final String eafPath, final Vector<String> mediaFiles) {
        this();
     ...
     */
    void openEAF(String fullPath, Vector<String> mediaFiles) {
        ...
        try {
            ...
            if (mediaFiles != null) {
                ...
                Vector<MediaDescriptor> descriptors = MediaDescriptorUtil.createMediaDescriptors(mediaFiles);
    ...
    public boolean checkMedia(Transcription transcription, String eafPath) {
        ...
        if (val instanceof Boolean) {
        	saveChangedMedia = ((Boolean) val).booleanValue();
        }
        ...
        try {
            Vector<MediaDescriptor> mediaDescriptors = transcription.getMediaDescriptors();

            for (int i = 0; i < mediaDescriptors.size(); i++) {
                MediaDescriptor md = mediaDescriptors.elementAt(i);
                ...
                if (!file.exists()) {
                    ...
                    if (val instanceof String) {
                    	file = new File(FileUtility.urlToAbsPath((String)val) + "/" + localFileName);
    ...
    */
   private boolean linkingTheSameMediaTwice(Vector<MediaDescriptor> mediaDescriptors, String absolutePath) {
     ...
     */
    private void adjustMediaDescriptors(Vector<MediaDescriptor> mediaDescriptors, int index,
        String newPath) {
        // remember the old URL
        String oldURL = mediaDescriptors.elementAt(index).mediaURL;
        ...
        mediaDescriptors.elementAt(index).mediaURL = newURL;
        mediaDescriptors.elementAt(index).mimeType = MediaDescriptorUtil.mimeTypeForExtension(newExt);

        // replace the extracted from URL's
        for (int i = 0; i < mediaDescriptors.size(); i++) {
            String extractedFrom = mediaDescriptors.elementAt(i).extractedFrom;

            if (oldURL.equals(extractedFrom)) {
                mediaDescriptors.elementAt(i).extractedFrom = newURL;
            }
        }
    }
    ...
    private void initElan() {
        ...
        // ELAN_MOD Start
        EventHook.getInstance().setPlayer(viewerManager);
        EventHook.getInstance().setCommunicationHandler();
        // ELAN_MOD End     
        ...
        for (int i = 0; i < transcriptionForThisFrame.getMediaDescriptors().size(); i++) {
          md = (MediaDescriptor) transcriptionForThisFrame.getMediaDescriptors().get(i);
          path = md.mediaURL;
          ...
          // audio detectors may work with video...
...
//        }
    ...
    private void initFrame() {
    	Object val = Preferences.get("ActivityMonitoring.AlwaysStartMonitoring", null);
    	if(val instanceof Boolean){
        ...
        Dimension d = (Dimension) Preferences.get("FrameSize", transcriptionForThisFrame);
        
        if (d == null) {
        	d = (Dimension) Preferences.get("FrameSize", null);
        }
...
//        }
        ...
        Point p = (Point) Preferences.get("FrameLocation", transcriptionForThisFrame);
        
        if (p == null) {
        	p = (Point) Preferences.get("FrameLocation", null);
        }
...
//        	}
    ...
    private void initMenuBar() {
       ...
       // JMenuItem menuItemMultipleFileAnnotationsFromOverlaps = new JMenuItem(ma);
        ...
        ma = new MultipleFileMergeTiersMA(ELANCommandFactory.MERGE_TIRES_MULTI, this);
        menuActions.put(ELANCommandFactory.MERGE_TIRES_MULTI, ma);
        mfProcessMenu.add(new JMenuItem(ma));
        ...
        menuItemAudioRecognizer = new JCheckBoxMenuItem(new CreateAudioRecognizerMA(ELANCommandFactory.AUDIO_RECOGNIZER,this));  
        menuItemAudioRecognizer.setSelected(true); 
        menuViewer.add(menuItemAudioRecognizer);   
        
        menuItemVideoRecognizer = new JCheckBoxMenuItem(new CreateVideoRecognizerMA(ELANCommandFactory.VIDEO_RECOGNIZER,this));      
        menuItemVideoRecognizer.setSelected(true); 
        menuViewer.add(menuItemVideoRecognizer);         
...
//        menuViewer.add(menuItemInterLinearizerViewer);
		...
		ma = new ShortcutsMA(ELANCommandFactory.SHORTCUTS);
		menuActions.put(ELANCommandFactory.SHORTCUTS, ma);
        menuView.add(ma);
        
        ma = new FontBrowserMA(ELANCommandFactory.FONT_BROWSER);
...
//        menuOptions.add(menuItemRateVol);
        	...
        	if(ca != null){
        		...
        		if(ksDescription != null && ksDescription.trim().length() > 0){
        			ca.putValue(Action.SHORT_DESCRIPTION,
        	                ElanLocale.getString(ca.getValue(Action.NAME) + "ToolTip"));
        		}
        	} else{
    ...
    public void updateShortcutMap(String modeConstant){
    	if(modeConstant != null && modeConstant.equals(ELANCommandFactory.INTERLINEARIZATION_MODE)){
    		return;
    	}
...
//        }     
        ...
        menuActions.remove(mergeTransMI.getAction());
        ...
        menuActions.remove(importPraatMI.getAction());
        ...
        menuActions.remove(importRecogTiersMI.getAction());
...
//        Object val = Preferences.get("LayoutManager.CurrentMode", viewerManager.getTranscription()); 
//        if(val != null){
//        	switch((Integer)val){
...
//        }
    ...
    private void loadViewerPreferences(){
    	...
    	if (val instanceof Boolean) {
    		menuItemGridViewer.setSelected((Boolean)val);    	
    	}
    	
    	val = Preferences.get(ELANCommandFactory.TEXT_VIEWER, null);
    	if (val instanceof Boolean) {
    		menuItemTextViewer.setSelected((Boolean)val);    	
    	}
    	
    	val = Preferences.get(ELANCommandFactory.SUBTITLE_VIEWER, null);
    	if (val instanceof Boolean) {
    		menuItemSubtitleViewer.setSelected((Boolean)val);    	
    	}
    	
    	val = Preferences.get(ELANCommandFactory.LEXICON_VIEWER, null);
    	if (val instanceof Boolean) {
    		menuItemLexiconViewer.setSelected((Boolean)val);    	
    	}
    	
    	val = Preferences.get(ELANCommandFactory.AUDIO_RECOGNIZER, null);
    	if (val instanceof Boolean) {
    		menuItemAudioRecognizer.setSelected((Boolean)val);    	
    	}
    	
    	val = Preferences.get(ELANCommandFactory.VIDEO_RECOGNIZER, null);
    	if (val instanceof Boolean) {
    		menuItemVideoRecognizer.setSelected((Boolean)val);    	
    	}
    	
    	val = Preferences.get(ELANCommandFactory.METADATA_VIEWER, null);
    	if (val instanceof Boolean) {
    		menuItemMetaDataViewer.setSelected((Boolean)val);    	
    	}
    	
    	val = Preferences.get(ELANCommandFactory.SIGNAL_VIEWER, null);
    	if (val instanceof Boolean) {
    		menuItemSignalViewer.setSelected((Boolean)val);    	
    	}    
    	val = Preferences.get(ELANCommandFactory.INTERLINEAR_VIEWER, null);
    	if (val instanceof Boolean) {
    		menuItemInterLinearViewer.setSelected((Boolean)val);    	
    	} 
    	val = Preferences.get(ELANCommandFactory.TIMESERIES_VIEWER, null);
    	if (val instanceof Boolean) {
    		menuItemTimeSeriesViewer.setSelected((Boolean)val);    	
    	} 
//    	val = Preferences.get(ELANCommandFactory.INTERLINEAR_LEXICON_VIEWER, null);
//    	if (val instanceof Boolean) {
...
//    	}
                ...
                public void run() {
                    ...
                    */
                    ...
                    if (timeScaleBeginTime != null) {
        	            ...
        	            if (val instanceof Boolean) {
        	                viewerManager.setFrameStepsToBeginOfFrame(((Boolean) val).booleanValue());
        	            }
                    /*
    ...
    private void loadCVPreferences() {
    	...
    	Object cvPrefs = Preferences.get(Preferences.CV_PREFS, transcriptionForThisFrame);
    	if (cvPrefs instanceof HashMap) {
    		...
    		for (int i = 0; i < allCV.size(); i++) {
    			...
    			if (cvMapObj instanceof HashMap) {
    				cvMap = (HashMap) cvMapObj;
    				
    				for (CVEntry cve : cv) {
    					String id = cve.getId();
    					if (cvMap.containsKey(id)) {
    						cveMap = (HashMap) cvMap.get(id);
...
//        	}
        ...
        if (val instanceof Integer && transcriptionForThisFrame != null) {
        ...
        val = Preferences.get("PlaybackRateToggleValue", null);
        if (val instanceof Float && transcriptionForThisFrame != null) {
        ...
        val = Preferences.get("PlaybackVolumeToggleValue", null);
        if (val instanceof Float && transcriptionForThisFrame != null) {
	            ...
	            storeComm.execute(transcriptionForThisFrame,
	                new Object[] {
	                    ...
	                    viewerManager.getMultiTierControlPanel()
	                                 .getVisibleTiers(),
	                    new Integer(TranscriptionStore.EAF)
	                });
            } else {
	            storeComm.execute(transcriptionForThisFrame,
		                new Object[] {
    ...
    public void doClose(boolean unregister) {
      // ELAN_MOD Start
      external.communication.EventHook eh = external.communication.EventHook.getInstance();
      eh.stopRPCConnection();
      // ELAN_MOD End
